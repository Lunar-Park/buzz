...
fn channel_builders_reject_hash_only_names() {
    ...
    assert!(build_update_channel(channel_id, Some("###"), None, None, None, None, None).is_err());
}
...
//
// The composer diffs the edited body's mentions against the original and hands
// `build_message_edit` only the newly added pubkeys. These tests pin the
// builder's contract given that contract: emit a `p` per added mention
// (deduped, lowercased), and none when the added set is empty (typo-fix edit)
// so an unchanged mention set re-wakes nobody.
