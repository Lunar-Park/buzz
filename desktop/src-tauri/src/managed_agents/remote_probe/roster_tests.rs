...
//!
//! The JSON in `LUNAR01_ROSTER` is the real shape returned by
//! `openclaw agents list --json` on OpenClaw 2026.7.1, captured read-only from a
//! live host. Parsing is pinned against observed output rather than a shape
//! inferred from documentation, because the compatibility spikes for this
//! integration repeatedly found the installed runtime and the docs disagreeing.

use super::*;

/// Verbatim excerpt of a real OpenClaw 2026.7.1 roster: a primary with no name,
/// plus named agents carrying identity metadata.
const LUNAR01_ROSTER: &str = r#"[
  {
    "id": "main",
    "workspace": "/Users/selene/.openclaw/workspace",
    "agentDir": "/Users/selene/.openclaw/agents/main/agent",
  ...
  },
  {
    ...
    "workspace": "/Users/selene/.openclaw/agents/astra/workspace",
  ...
  },
  {
    "id": "cassian",
    "name": "Cassian",
    "identityName": "Cassian",
    "workspace": "/Users/selene/.openclaw/agents/cassian/workspace",
    "model": "minimax/MiniMax-M2.7",
    "bindings": 1,
    "isDefault": false
  }
...
fn parses_a_real_openclaw_roster() {
    let candidates = parse_openclaw_roster(LUNAR01_ROSTER).expect("roster parses");
...
fn main_is_primary_when_the_harness_flags_nothing() {
    ...
    let payload = r#"[{"id":"astra","name":"Astra"},{"id":"main"}]"#;
    let candidates = parse_openclaw_roster(payload).expect("parses");
    let main = candidates
        .iter()
...
fn no_primary_is_claimed_when_there_is_no_default_and_no_main() {
    ...
    // Buzz guessed at.
    let payload = r#"[{"id":"astra","name":"Astra"},{"id":"cato","name":"Cato"}]"#;
    let candidates = parse_openclaw_roster(payload).expect("parses");
...
fn an_explicit_default_beats_the_main_fallback() {
    let payload = r#"[{"id":"main"},{"id":"astra","isDefault":true}]"#;
    let candidates = parse_openclaw_roster(payload).expect("parses");
    assert_eq!(candidates[0].agent_id, "astra");
    assert!(candidates[0].is_primary);
    let main = candidates
        .iter()
        .find(|c| c.agent_id == "main")
...
fn rows_without_a_routable_id_are_dropped() {
    let payload = r#"[{"id":"  "},{"id":"astra","name":"Astra"}]"#;
    let candidates = parse_openclaw_roster(payload).expect("parses");
    assert_eq!(candidates.len(), 1);
    assert_eq!(candidates[0].agent_id, "astra");
}
...
fn blank_optional_details_become_none_rather_than_empty_strings() {
    let payload = r#"[{"id":"astra","model":"","workspace":"   "}]"#;
    let candidates = parse_openclaw_roster(payload).expect("parses");
...
fn binding_count_is_carried_through() {
    let candidates = parse_openclaw_roster(LUNAR01_ROSTER).expect("parses");
    let cassian = candidates
        .iter()
        .find(|c| c.agent_id == "cassian")
        .expect("cassian present");
    assert_eq!(cassian.binding_count, Some(1));
}
