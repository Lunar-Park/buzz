...
fn classifies_a_first_seen_key_under_strict_checking() {
    ...
    assert_eq!(
        classify_ssh_failure(
            "No ED25519 host key is known for lunar02 and you have requested strict \
...
fn an_untrusted_key_and_a_changed_key_read_differently() {
    ...
    let unknown = failure_message(
        ...
        "lunar02",
        "No ED25519 host key is known for lunar02 and you have requested strict checking.",
    );
    ...
    assert!(
        unknown.contains("not yet trusted") && unknown.contains("ssh lunar02"),
...
fn probe_with_stdout(script: &str) -> HostProbeResult {
    ...
    run_probe(command, "lunar02", &targets(), Instant::now())
}
