...
fn failure_message(kind: &HostProbeErrorKind, host: &str, stderr: &str) -> String {
    match kind {
        ...
        HostProbeErrorKind::HostKeyProblem => format!(
            "The host key for '{host}' could not be verified. Resolve the trust decision \
             outside Buzz (connect once with `ssh {host}` and review the key), then probe again."
        ),
...
pub fn probe_ssh_host(host: &SshHost) -> HostProbeResult {
    ...
    command
        ...
        .arg("-o")
        .arg("BatchMode=yes")
        // Do not silently trust a changed key; an unknown key is a reportable
        // status, not something to accept on the user's behalf.
        .arg("-o")
    ...
    command.arg(&host.host).arg(&script);
...
mod tests {
    ...
    fn parses_binaries_user_host_and_os() {
        ...
        assert_eq!(facts.hostname.as_deref(), Some("lunar02"));
    ...
    fn password_message_names_the_remedy_and_promises_no_storage() {
        ...
        assert!(message.contains("ssh-copy-id lunar02"));
    }
