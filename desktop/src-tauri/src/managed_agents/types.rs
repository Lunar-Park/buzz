...
// The inbound author gate (`RespondTo`, allowlist validation) lives in its
// own submodule; re-exported so existing `types::RespondTo` paths still work.
mod respond_to;
pub use respond_to::{validate_respond_to_allowlist, RespondTo};
