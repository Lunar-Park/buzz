...
fn records_round_trip_through_the_store() {
    let dir = tempfile::tempdir().expect("temp dir");
    let path = dir.path().join("connected-agents.json");
    let records = vec![connected(CONNECTED_HEX, "Scout", "lunar02")];
...
fn a_first_save_creates_the_store_and_a_second_overwrites_it_atomically() {
    ...
    save_connected_agents_at(&path, &[connected(CONNECTED_HEX, "Scout", "lunar02")])
        .expect("first");
    save_connected_agents_at(&path, &[connected(CONNECTED_HEX, "Scout", "lunar03")])
        .expect("second");

    let loaded = load_connected_agents_at(&path).unwrap();
    assert_eq!(loaded.len(), 1);
    assert_eq!(loaded[0].host, "lunar03");
    assert!(
...
fn records_are_sorted_for_stable_diffs() {
    ...
    let path = dir.path().join("connected-agents.json");
    let records = vec![
        connected(CONNECTED_HEX, "zeta", "lunar02"),
        connected(OWNED_HEX, "Alpha", "lunar03"),
    ];
...
fn a_connected_record_cannot_be_deserialized_as_a_managed_record() {
    ...
    let record = connected(CONNECTED_HEX, "Scout", "lunar02");
...
fn the_two_stores_are_separate_files_and_a_connected_save_leaves_the_other_untouched() {
    ...
    save_connected_agents_at(
        &connected_path,
        &[connected(CONNECTED_HEX, "Scout", "lunar02")],
    )
...
fn a_connected_store_write_carries_no_secret_and_needs_no_restricted_mode() {
    ...
    let dir = tempfile::tempdir().expect("temp dir");
    let path = dir.path().join("connected-agents.json");
    save_connected_agents_at(&path, &[connected(CONNECTED_HEX, "Scout", "lunar02")]).expect("save");
